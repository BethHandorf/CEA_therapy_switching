...
#This version is for strategy 1 (Docetaxel first)
...
#setwd("U:\\Geynisman_D\\Prostate_Markov_Model\\2 line model\\")
...
getSSE<-function(x){ 
  ...
  sim_DCT_AB  <- MicroSim(v.M_1, n.i, n.t, v.n, d.c, d.e, Trt = 1, seed=fmsfundata$seed1*100000)  
  
  #output matrix - get how many patients are in each state by cycle
  tr.DCT_AB<-data.frame(sim_DCT_AB$TR)
  
  #compare the the target survival data
  trial.surv<-survfit(Surv(Time*(30.4/21),Event)~1, data=OS.DCT)
  cdf.trial<-1-summary(trial.surv, times = c(0:87))[['surv']]
  
  #Sum of squared errors
  ss_err<-sum((cdf.trial- tr.DCT_AB$Death)^2)
...
nm <- neldermead.set(nm,'boundsmin',c(log(0.01),log(0.01)))
nm <- neldermead.set(nm,'boundsmax',c(log(10),log(87)))  #Max allowable HRs
...
write.csv(nm$optbase$xopt,"Opt_parms_DCTfirst.csv",append=TRUE)
